// Q-Learning Algorithm (Off-Policy Value-Based)

import { RLAlgorithm } from './base';
import { GridWorld, GridWorldState } from '../environments/gridworld';

export class QLearning implements RLAlgorithm {
  ...
  update(state: GridWorldState, action: number, reward: number, nextState: GridWorldState, done: boolean): void {
    ...
    if (!done) {
      const nextKey = this.env.getStateKey(nextState);
      if (this.Q.has(nextKey)) {
        nextMaxQ = Math.max(...this.Q.get(nextKey)!);
      }
    }

    const targetQ = reward + this.discountFactor * nextMaxQ;
