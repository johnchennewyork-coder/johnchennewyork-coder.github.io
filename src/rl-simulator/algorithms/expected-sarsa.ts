// Expected SARSA Algorithm (On-Policy Value-Based)

import { RLAlgorithm } from './base';
import { GridWorld, GridWorldState } from '../environments/gridworld';

export class ExpectedSARSA implements RLAlgorithm {
  ...
  private getExpectedQ(state: GridWorldState): number {
    const key = this.env.getStateKey(state);
    if (!this.Q.has(key)) {
      this.Q.set(key, new Array(this.env.getNumActions()).fill(0));
    }
    ...
    const bestActions = qValues.filter(q => q === maxQ).length;
    const numActions = this.env.getNumActions();

    // Epsilon-greedy expected value
    const probBest = (1 - this.epsilon) / bestActions + this.epsilon / numActions;
    const probOther = this.epsilon / numActions;
  ...
  update(state: GridWorldState, action: number, reward: number, nextState: GridWorldState, done: boolean): void {
    ...
    const nextExpectedQ = done ? 0 : this.getExpectedQ(nextState);

    const targetQ = reward + this.discountFactor * nextExpectedQ;
